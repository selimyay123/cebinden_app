// ignore_for_file: equal_keys_in_map, unused_element, empty_catches
...
class OfferService {
    ...
    try {
      ...
      for (int i = 0; i < buyerCount; i++) {
        ...
        if (interested) {
          ...
          Offer offer = Offer(
            offerId:
                'offer_${DateTime.now().millisecondsSinceEpoch}_${Random().nextInt(10000)}',
            vehicleId: listing
                .id, // ✅ UserVehicle'ın ID'sini kullan (listing.vehicleId değil!)
            ...
            message:
                buyer.generateContextualMessage(
      ...
      if (offersCreated == 0 && buyerCount > 0) {
        ...
        Offer offer = Offer(
          ...
          status: OfferStatus.pending,
          message:
              forcedBuyer.generateContextualMessage(
                mileage: listing.mileage,
                hasAccidentRecord: listing.hasAccidentRecord,
                listingPrice: listing.listingPrice!,
                fairPrice: fairPrice,
  ...
  Future<XPGainResult?> acceptOffer(Offer offer) async {
    try {
      ...
      final xpResult = await _processIncomingOfferAcceptance(
        offer,
        offer.counterOfferAmount ?? offer.offerPrice,
      );
      ...
      await NotificationService().deleteNotificationsForVehicle(
        offer.vehicleId,
      );
  ...
  Future<Map<String, dynamic>> submitUserOffer({
    ...
    try {
      ...
      if (userMapCheck != null) {
        ...
        if (userVehicles.length >= user.garageLimit) {
          return {
            'success': false,
            'error':
                'Garaj limitiniz dolu (${user.garageLimit} araç). Yeni araç alamazsınız.',
          };
        }
      }
      ...
      final hasRejectedOffer = existingOffers.any(
        (o) => o.buyerId == userId && o.status == OfferStatus.rejected,
      );

      if (hasRejectedOffer) {
        return {
          'success': false,
          'error': 'offer.previouslyRejected'
              .tr(), // "Bu araç için yapılan karşı teklifi reddettiniz. Tekrar teklif veremezsiniz."
        };
      }
      ...
      final offer = Offer(
        offerId:
            'user_offer_${DateTime.now().millisecondsSinceEpoch}_${Random().nextInt(10000)}',
  ...
  Future<Map<String, dynamic>> submitCounterOfferResponse({
    ...
    try {
        ...
        if (previousCounterOffer != null && newCounterAmount != null) {
          if (newCounterAmount >= previousCounterOffer) {
            ...
            final lowerAmount =
                ...
                (previousCounterOffer *
                    (0.02 + random.nextDouble() * 0.03)); // %2-5 daha düşük
      ...
      return {
        'success': true,
        'decision':
            decision, // Orijinal kararı döndür (UI için) - veya yeni kararı? UI result dialogu için yeni karar daha iyi olabilir ama orijinal yapı bozulmasın.
        ...
        'decision': newStatus == OfferStatus.accepted
            ? 'accept'
            : (newStatus == OfferStatus.rejected ? 'reject' : 'counter'),
        'status': newStatus,
  ...
  Future<Map<String, dynamic>> acceptCounterOffer(Offer offer) async {
    try {
      ...
      final success = await _processUserOfferAcceptance(
        updatedOffer,
        offer.buyerId,
      );
      ...
      if (success) {
        ...
        return {
          'success': false,
          'error': 'Satın alma işlemi başarısız (Bakiye yetersiz olabilir)',
        };
      }
    } catch (e) {
      return {'success': false, 'error': e.toString()};
  ...
  Future<Map<String, dynamic>> sendCounterOfferToIncomingOffer({
    ...
    try {
      ...
      final aiBuyer = AIBuyer.generateRandom(
        seed: originalOffer.offerId.hashCode,
      );
      ...
      if (decision['decision'] == 'accept') {
        ...
        await _processIncomingOfferAcceptance(
          originalOffer,
          counterOfferAmount,
        );
      } else {
  ...
  Future<Map<String, dynamic>> _evaluateCounterOfferByBuyer({
    ...
    required double
    counterOfferAmount, // Kullanıcının istediği fiyat (User's Ask)
    ...
    final aggressiveness = _getAggressivenessFromBuyerType(
      aiBuyer.buyerType,
      seller: seller,
    );
    ...
    if (counterOfferAmount <= originalOfferPrice) {
      return {'decision': 'accept', 'response': _generateAcceptanceResponse()};
    }
    ...
    if (priceRatio >= 0.90) {
      ...
      if (initialOfferRatio < 0.90) {
        ...
        if (random.nextDouble() < 0.1 + successBonus) {
          // %10 şans
          return {
            'decision': 'accept',
    ...
    if (priceRatio >= 0.95) {
      ...
      if (random.nextDouble() < 0.3 + successBonus) {
        ...
        // Reddet
        return {'decision': 'reject', 'response': _generateRejectionResponse()};
      }
      ...
      // İyi fiyat (%70-85)
      // Kabul şansı yüksek
      if (random.nextDouble() < 0.8 + successBonus) {
        // %80 şansla kabul
        return {
          'decision': 'accept',
          'response': _generateAcceptanceResponse(),
  ...
  Future<XPGainResult?> _processIncomingOfferAcceptance(
    Offer offer,
    double finalPrice,
  ) async {
    try {
  ...
  Future<bool> _processUserOfferAcceptance(Offer offer, String userId) async {
    try {
      ...
      await _db.updateUser(userId, {
        'balance': user.balance - offer.offerPrice,
      });
      ...
      final userVehicle = UserVehicle.purchase(
        ...
        originalListingPrice:
            offer.listingPrice, // 🆕 Orijinal ilan fiyatını kaydet
      );
  ...
  double _getAggressivenessFromBuyerType(BuyerType type, {User? seller}) {
    ...
    switch (type) {
      case BuyerType.bargainer:
        aggressiveness =
            0.8; // Yüksek agresiflik - pazarlığa devam etmeye eğilimli
        break;
  ...
  Future<int> _calculateDailyBuyerCount(UserVehicle listing) async {
    ...
    if (listing.listedDate != null) {
      final daysSinceListed = DateTime.now()
          .difference(listing.listedDate!)
          .inDays;

      if (daysSinceListed > 30) {
        baseCount = (baseCount * 0.7)
            .round(); // %30 azalt (daha az cezalandırıcı)
