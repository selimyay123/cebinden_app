...
class SettingsScreen extends StatefulWidget {
  ...
  const SettingsScreen({super.key, this.highlightProfilePicture = false});
  ...
  void initState() {
    ...
    _highlightAnimation =
        ColorTween(
          ...
          end: Colors.deepPurple.withValues(alpha: 0.3),
        ).animate(
          CurvedAnimation(
  ...
  Future<void> _loadSettings() async {
    try {
      ...
      setState(() {
        _darkMode = darkMode;
        _selectedLanguage =
            _localizationService.currentLanguage; // Mevcut dili yükle
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          content: Text(
            'settings.gameDayDurationUpdated'.trParams({
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) {
          return ModernAlertDialog(
                ...
                itemBuilder: (context, index) {
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: UserProfileAvatar(imageUrl: imagePath, radius: 30),
                    ),
                  );
                },
              ),
            ...
            onPressed:
                selectedImage != null &&
                    selectedImage != _currentUser?.profileImageUrl
                ? () {
    ...
    if (confirm == true) {
      try {
        ...
        if (success) {
          ...
          // Genel hata
          _showDeleteError('settings.deleteAccountError'.tr());
        }
      } catch (e) {
        if (!mounted) return;
        // Debug Log

        // Yeniden giriş hatası kontrolü
        if (e.toString().contains('requires-recent-login')) {
          // Re-auth Dialog
          final reauthSuccess = await showDialog<bool>(
            context: context,
            barrierDismissible: false,
            builder: (context) => ReauthenticationDialog(user: _currentUser!),
          );

          if (reauthSuccess == true) {
            // Re-auth başarılı, tekrar silmeyi dene
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('settings.confirmDeleteAfterAuth'.tr()),
            ...
            try {
              final successRetry = await _authService.deleteAccount(
                _currentUser!.id,
              );
              if (!mounted) return;
              ...
              _showDeleteError(
                '${'settings.deleteAccountError'.tr()}: $retryError',
              );
            }
          }
        } else {
          _showDeleteError('${'settings.deleteAccountError'.tr()}: $e');
        }
      }
    }
  }
        ...
        return Scaffold(
          ...
          body: ListView(
            children: [
              ...
              if (_currentUser != null &&
                  _authService.isUserAdmin(_currentUser!))
                Container(
                  ...
                  _buildListTile(
                    ...
                    onTap: () {
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) =>
                              ProfileInfoScreen(user: _currentUser!),
                        ),
                      );
                    },
                  ),
                  ...
                  _buildListTile(
                    ...
                    onTap: () {
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) =>
                              ChangePasswordScreen(userId: _currentUser!.id),
                        ),
                      );
                    },
                  ),
                  ...
                  _buildListTile(
                    ...
                    trailing: DropdownButton<String>(
                      ...
                      items: [
                        DropdownMenuItem(
                          value: 'tr',
                          child: Text('settings.turkish'.tr()),
                        ),
                        DropdownMenuItem(
                          value: 'en',
                        ...
                        DropdownMenuItem(
                          value: 'es',
                          child: Text('settings.spanish'.tr()),
                        ),
                      ],
                  ...
                  _buildListTile(
                    ...
                    trailing: DropdownButton<int>(
                      ...
                      items: [
                        DropdownMenuItem(
                          value: 1,
                          child: Text('1 ${'misc.minutes'.tr()}'),
                        ),
                        DropdownMenuItem(
                          value: 2,
                          child: Text('2 ${'misc.minutes'.tr()}'),
                        ),
                        DropdownMenuItem(
                          value: 5,
                          child: Text('5 ${'misc.minutes'.tr()}'),
                        ),
                        DropdownMenuItem(
                          value: 10,
                          child: Text('10 ${'misc.minutes'.tr()}'),
                        ),
                        DropdownMenuItem(
                          value: 15,
                          child: Text('15 ${'misc.minutes'.tr()}'),
                        ),
                      ],
